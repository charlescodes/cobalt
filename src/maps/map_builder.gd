...
const STATIC_GROUND_NAME: StringName = &"StaticGround"
const STATIC_WALLS_NAME: StringName = &"StaticWalls"
const WORLD_OBJECTS_NAME: StringName = &"WorldObjects"
const GROUND_MOVE_TARGET_THICKNESS_M: float = 0.02
...
static func _add_roots(root: Node3D, map_data: MapDataScript) -> void:
	var ground_root := _new_root(STATIC_GROUND_NAME)
	...
	for ground_index in range(map_data.grounds.size()):
		_add_ground(ground_root, map_data.grounds[ground_index], ground_index)
...
static func _add_ground_move_target(parent: Node3D, ground: GroundDataScript) -> void:
	...
	var collision := CollisionShape3D.new()
	collision.name = "CollisionShape3D"
	var target_height := minf(ground.size_m.y, GROUND_MOVE_TARGET_THICKNESS_M)
	collision.position = Vector3(0.0, (ground.size_m.y * 0.5) - (target_height * 0.5), 0.0)
	var box_shape := BoxShape3D.new()
	box_shape.size = Vector3(ground.size_m.x, target_height, ground.size_m.z)
	collision.shape = box_shape
...
static func _add_wall(parent: Node3D, segment: WallSegmentDataScript, wall_index: int) -> void:
	...
	wall_body.name = "Wall_%02d" % wall_index
	wall_body.position = WallVisualResolverScript.visual_center(segment)
	wall_body.rotation.y = WallVisualResolverScript.visual_rotation_y(segment)
